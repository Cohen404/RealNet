...
def update_config(config_path, use_rl=False, rl_config_path=None):
...
def load_checkpoint(model, checkpoint_path, device, use_rl=False):
    ...
    if use_rl and hasattr(model.module, 'afs'):
        if 'rl_state_dict' in checkpoint:
            print("Loading RL state")
            model.module.afs.load_rl_state_dict(checkpoint['rl_state_dict'])
        
        # Load PPO policy if available
        if 'ppo_policy' in checkpoint and hasattr(model.module.afs, 'load_ppo_policy'):
            print("Loading PPO policy for inference")
            model.module.afs.load_ppo_policy(checkpoint['ppo_policy'])
            # Enable dynamic inference if configured
            if hasattr(model.module.afs, 'rl_config') and model.module.afs.rl_config.get('dynamic_inference', False):
                print("Enabling dynamic inference with RL agent")
                model.module.afs.enable_ppo_inference()
...
def main():
    ...
    # Update config
    config = update_config(args.config, args.use_rl, args.rl_config)
